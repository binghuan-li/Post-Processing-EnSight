...
ensight.variables.activate(f"{var_names['wss']}");
ensight.variables.evaluate(f"user_rms_wall_shear = RMS({var_names['wss']})");
ensight.part.select_begin(wall_part_ID);
expr = "user_TAWSS = TempMean(plist,user_rms_wall_shear,{},{})".format(start_pt, end_pt);
err = ensight.variables.evaluate(expr);
if err == 0:
    print("\t--> TAWSS calculation successful!");
else:
    raise Exception("Calculation in TAWSS failed.");
